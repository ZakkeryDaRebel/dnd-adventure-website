...
const homePage: string =
  "http://localhost:5179/solo-adventures/dnd-adventure-website/#/";
...
test("can load", async ({ page }) => {
  ...
  await expect(
    page.getByRole("button", { name: "The Dark Forest" }),
  ).toBeVisible();
  await expect(
...
test("adventure buttons are clickable", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "The Dark Forest" }).click();
  await expect(page).toHaveURL(homePage + "adventure/dark-forest");
